...
def process_P_Query(query):
    ...
    if '|' not in content:  # joint/ marginal distribution
        ...
        prob_enumeration, prob_elimination = conditional_probability(
            x_group, parent_vars, all_evidence, parent_evidence, children_TorF_tuple)
...
# The function below are to be implemented
...
def enumeration_ask(X, e, bn):
    ...
    Q = ProbDist(X)
    # YOUR_CODE_HERE (hint: only two-four lines, fig14.9)
    for x in bn.variable_values(X):
        Q[x] = enumerate_all(bn.variables, extend(e, X, x), bn)
    return Q.normalize()
...
def elimination_ask(X, e, bn, order=reversed):
    # YOUR_CODE_HERE 
    # Hint the makefactor, sum_out and pointwise_product are already implemented
    factors = []
    for var in reversed(bn.variables):
        factors.append(make_factor(var, e, bn))
        if var not in e.keys() and var not in X:
...
# The function below will be directly used in your implementation
...
def sum_out(var, factors):
    # var: hidden variable that are ready to be sum out e.g. 'MaryCall'
    ...
    cpt = {event_values(e, variables): sum(sum_factor.p(extend(e, var, val)) for val in [True, False])
...
def pointwise_product(factors):
    ...
    for i in range(len(factors)-1):
        ...
        cpt = {event_values(e, variables): result_factor.p(
            e)*factors[i+1].p(e) for e in all_events(variables, {})}
